...
def longest_progression():
    ...
    result = 0
    for i in xrange(N):
        dp[i%4], dp_skip[i%4] = min(i+1, 2), min(i+1, 2+1)
        if i >= 2 and A[i]-A[i-1] == A[i-1]-A[i-2]:  # no replace
            dp[i%4] = dp[(i-1)%4]+1
            dp_skip[i%4] = dp_skip[(i-1)%4]+1
        if i >= 3 and A[i]-A[i-2] == 2*(A[i-2]-A[i-3]):  # replace A[i-1]
            result = max(result, dp[(i-2)%4]+1+1)
        if i >= 3 and 2*(A[i]-A[i-1]) == A[i-1]-A[i-3]:  # replace A[i-2]
